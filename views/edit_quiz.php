...
<hr>
	...
	?>
					<div class="form-group">	 
				 	<select   name="cid[]" id="cid" onchange="selectSubSkill(this.value);">
					<option value="0"><?php echo $this->lang->line('select');?> <?php echo $this->lang->line('category_name');?></option>
					<?php 
					foreach($category_list as $key => $val){
						?>
...
<script>
function selectSubSkill(cid)
{
	
	//alert(cid);
   $.ajax({
  type: 'POST',
  url: "<?php echo base_url();?>index.php/quiz/getSubSkillFromCategoryID",
  data:{cid:cid}, 

  success: function (response1) {
	//alert(response1);  
	$("#skill_div").html(response1);
